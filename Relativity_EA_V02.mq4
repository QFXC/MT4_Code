...
//enter_order
	...
	input int enteringMaxSlippage=5; // the default used to be 50
...
//exit order
	input int exitingMaxSlippage = 50 // i added this variable
...
void enter_order(ENUM_ORDER_TYPE type)
  {
   ...
   entry(NULL,type,lots,0,enteringMaxSlippage,stoploss,takeprofit,order_comment,order_magic,order_expire,arrow_color_short,market_exec); // why does it only use the arrow_color_short variable and arrow_color_long is never used?
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
bool exit_order(int ticket,double size=-1,color a_color=clrNONE,int exitingMaxSlippage)
  {
   bool result=false;
   if(OrderSelect(ticket,SELECT_BY_TICKET))
     {
      if(OrderType()<=1)
        {
         result=OrderClose(ticket,OrderLots(),OrderClosePrice(),exitingMaxSlippage,a_color);
        }
      else if(OrderType()>1)
        {
...
bool exit(int ticket,color a_color=clrNONE,int exitingMaxSlippage,int retries=3,int sleep=500)
  {
   bool result=false;
   for(int i=0;i<retries;i++)
     {
      ...
      else result=exit_order(ticket,a_color,exitingMaxSlippage);
      if(result)
         break;
...
int send_order(string instrument,int cmd,double lots,int distanceFromCurrentPrice,int enteringMaxSlippage,int sl,int tp,string comment=NULL,int magic=0,int expire=0,color a_clr=clrNONE,bool market=false)
  {
   double entryPrice = 0; 
   double price_sl = 0; 
   double price_tp = 0;
   ...
   if(market)
     {
      int ticket=OrderSend(instrument,order_type,lots,entryPrice,enteringMaxSlippage,0,0,comment,magic,expiry,a_clr);
      if(ticket>0)
        {
         if(OrderSelect(ticket,SELECT_BY_TICKET))
           {
   ...
   return OrderSend(instrument,order_type,lots,entryPrice,enteringMaxSlippage,price_sl,price_tp,comment,magic,expiry,a_clr);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+

int entry(string instrument,int cmd,double lots,int distanceFromCurrentPrice,int enteringMaxSlippage,int sl,int tp,string comment=NULL,int magic=0,int expire=0,color a_clr=clrNONE,bool market=false,int retries=3,int sleep=500)
  {
   int ticket=0;
   for(int i=0;i<retries;i++)
     {
      ...
      else ticket=send_order(instrument,cmd,lots,distanceFromCurrentPrice,enteringMaxSlippage,sl,tp,comment,magic,expire,a_clr,market);
      if(ticket>0)
         break;
