...
// Market Trends
  ...
  extern string     trade_triggering_timeframe="H1";    // trade_triggering_timeframe: M5,H1
  ...
  static double     HOP_price,LOP_price;
  static datetime   HOP_time,LOP_time;
  static int        moves_start_bar;                    // the periods_pivot_price function uses this
  static ENUM_TIMEFRAMES moves_start_bar_tf;
...
// Identifying trends that last for a specified number of periods
...
int on_initialization(ENUM_SIGNAL_SET signal_set,string instrument)
  {
    ...
    ran=ran(ran,set_recommended_input_parameters(instrument,use_recommended_settings,settings_set));
    Print(ran," 3");
    //EventSetTimer(60);
    ...
    Print(ran," 6");
    ran=ran(ran,set_moves_start_bar(instrument,moves_start_bar_tf,hours_to_roll(),gmt_hour_offset,max_weekend_gap_percent,include_last_week));
    Print(ran," 7");
    ...
    set_ADR_pts(hours_to_roll(),below_ADR_outlier_percent,above_ADR_outlier_percent,change_ADR_percent,instrument,ADR_pts_raw); // this should be after the recommended settings are set because, if requested, a change should be done after
...
bool input_parameters_valid() // TODO: add more
  {
    ...
    if(exit_time>range_start_time && exit_time>=range_end_time)
      if(risk_percent_per_range>0 && risk_percent_per_range<.1) 
        return true; 
    return false;
  }
//+------------------------------------------------------------------+
...
void OnTick()
  {
    ...
    if(is_new_M5_bar)
      {
        is_new_H1_bar=is_new_H1_bar(instrument,false);
        ...
        if(is_new_H1_bar) 
          {
            ...
            //if(is_new_D1_bar) get_previous_days_bar_info_testing(instrument);
            ...
            if(is_new_custom_D1_bar)  // this must run before relativity_ea_ran function is run
              {
                ...
                //set_custom_W1_times(instrument,include_last_week,hours_to_roll(),gmt_hour_offset);
    ...
    if(safe_to_trade && trading_paused==false) 
      {
        analyze_market_for_trading(instrument,magic_num,digits,current_time,current_bid);
      }
    ...
    if(is_new_D1_bar || is_new_custom_D1_bar) 
        safe_to_trade=true; // by setting safe_to_trade=true, you are letting it be re-evaluated on the first tick for each day's broker and custom bar
    if(is_new_M5_bar==true) // Since the functions that return the true/false of whether it is a new bar do not run on every tick, the global variables need to be set to false after all the previous code gets run. Otherwise, many of them will stay true much too frequently.
      {
        is_new_M5_bar=false;
        is_new_H1_bar=false;
        is_new_D1_bar=false;
...
//|                                                                  |
//+------------------------------------------------------------------+
void analyze_market_for_trading(string instrument,int magic,int digits,datetime current_time,double current_bid)
  {
    bool a_trade_closed=false;
    ...
    // virtualstop_check(virtual_sl,virtual_tp); // this is where the virtualstop_check function will be called if you decide to implement it
    if(is_new_M5_bar)
      {
        a_trade_closed=a_trade_closed_since_last_checked(magic);
        ...
        daily_ready_check(instrument,current_time,magic);
      }
        ...
        // this Friday exit time code has to go before the daily exit time code in case the user wants to exit all trades on Fridays earlier than daily.
          {
            ...
            if(fri_time_to_exit) exit_all_trades_set(exiting_max_slippage_pips,ORDER_SET_ALL,magic); // this is the special case where you can exit open and pending trades based on a specified time (this should have been set to be outside of the trading time range)
          }
        else if(exit_trades_EOD && is_new_M5_bar)
          {
            ...
            if(daily_time_to_exit) exit_all_trades_set(exiting_max_slippage_pips,ORDER_SET_ALL,magic); // this is the special case where you can exit open and pending trades based on a specified time (this should have been set to be outside of the trading time range)
          }
        if(is_new_M5_bar) ea_retracement_maintenance(instrument,current_time,current_bid,a_trade_closed,in_time_range);  // SPECIFIC TO RETRACEMENT EA
      }
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void daily_ready_check(string instrument,datetime current_time,int magic)
  {
...
//|                                                                  |
//+------------------------------------------------------------------+
void ea_retracement_maintenance(string instrument,datetime current_time,double current_bid,bool a_trade_closed,bool _in_time_range)
  {
    string current_chart=Symbol();
    ...
    if(_in_time_range)
      {
        if(is_new_M5_bar) 
          {
            //retracement_ea_pivot_peak_reset_check
            ...
            if(_display_chart_objects)
              {
                ...
                if(is_new_H1_bar) // NOT SPECIFIC TO THE RETRACEMENT EA
                  {
                    double bid_price=current_bid;
                    string dow=current_chart+"_day_of_week";
                    if(ObjectFind(dow)<0)
                      {
                        ObjectCreate(dow,OBJ_TEXT,0,TimeCurrent(),bid_price);
                        ObjectSetText(dow,"0",15,NULL,clrWhite);
                      }
                    ObjectSetText(dow,IntegerToString(DayOfWeek(),1),0);
                    if(uptrend) ObjectMove(dow,0,TimeCurrent(),bid_price-ADR_pts_raw/2);
                    else ObjectMove(dow,0,TimeCurrent(),bid_price+ADR_pts_raw/2);                  
                  }
              }
          }
        if(_display_chart_objects) // NOT SPECIFIC TO THE RETRACEMENT EA
          {
        ...
        if(is_new_M5_bar) 
          {
            if(_display_chart_objects)
              {
                if(ObjectFind(current_chart+"_HOP")>=0) // NOT COMPLETELY SPECIFIC TO THE RETRACEMENT EA AND CAN BE APPLIED TO THE REVERSAL EA
                  {
...
//|                                                                  |
//+------------------------------------------------------------------+
ENUM_DIRECTION_BIAS ea_retracement_signal(string instrument,int digits,double current_bid)
  {
...
//|                                                                  |
//+------------------------------------------------------------------+
ENUM_DIRECTION_BIAS ea_reversal_signal(string instrument,int magic,int digits,double current_bid)
  {
    ENUM_DIRECTION_BIAS enter_signal=DIRECTION_BIAS_NEUTRALIZE; // 0
    if(is_new_M5_bar)
      {
        ENUM_DIRECTION_BIAS pinbar_signal=signal_is_pin_bar(instrument,PERIOD_M5,1,MathMax(ADR_pts_raw/10,ADR_pts));
        if(pinbar_signal>0) enter_signal=signal_bias_compare(pinbar_signal,ea_reversal_overextended_trend(instrument,current_bid,.75));
      }
    return enter_signal;
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
ENUM_DIRECTION_BIAS ea_reversal_overextended_trend(string instrument,double current_bid,double _ADR_pts_raw_multiplier)
  {
    double ma_price=iMA(instrument,PERIOD_M5,250,0,MODE_SMA,PRICE_MEDIAN,0);
    double distance=ma_price-current_bid;
    if(uptrend_order_was_last==false && distance>ADR_pts_raw*_ADR_pts_raw_multiplier) 
      {
        downtrend=true;
        uptrend=false;
        //moves_start_bar=iBarShift(instrument,PERIOD_M5,iTime(instrument,PERIOD_M5,1));
        HOP_price=ma_price;
        HOP_time=iTime(instrument,PERIOD_M5,1);
        LOP_price=iLow(instrument,PERIOD_M5,1);
        LOP_time=HOP_time;
        return DIRECTION_BIAS_BUY;
      }
    else if(downtrend_order_was_last==false && distance<-ADR_pts_raw*_ADR_pts_raw_multiplier) 
      {
        downtrend=false;
        uptrend=true;
        //moves_start_bar=iBarShift(instrument,PERIOD_M5,iTime(instrument,PERIOD_M5,1));
        LOP_price=ma_price;
        LOP_time=iTime(instrument,PERIOD_M5,1);
        HOP_price=iHigh(instrument,PERIOD_M5,1);
        HOP_time=LOP_time;
...
//|                                                                  |
//+------------------------------------------------------------------+
ENUM_DIRECTION_BIAS ea_pinbar_signal(string instrument,int digits,double current_bid)
  {
    // start to filter and generally look for the buy or sell enter signals before specifically analyzing the buy and sell signals
    ENUM_DIRECTION_BIAS enter_signal=DIRECTION_BIAS_NEUTRALIZE; // 0
    /*
...
ENUM_ORDER_TYPE general_filters(string instrument,int digits,ENUM_DIRECTION_BIAS enter_signal,int magic,datetime current_time,double current_bid)
  {
    ...
    if(enter_signal>0 && diff_moving_avg_period>0 && past_x_periods_same_trend>0 && ma_diff_multiplier>0)
      {
    ...
    { // keep the if statements in this block in this order
    ...
    if(enter_signal==DIRECTION_BIAS_BUY && long_allowed)
      {
        ...
        else ea_retracement_maintenance(instrument,0,0,0,false); // this will delete the chart objects
      }
...
bool is_previous_D1_bar_correct_duration(string instrument,datetime current_time)
  {
    ...
    print_and_email("Warning","TRADING WILL NOT HAPPEN TODAY BECAUSE THE PREVIOUS D1 BAR WAS NOT 24 HOURS LONG");
    return false;
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
ENUM_DIRECTION_BIAS signal_ADR_triggered(string instrument,string _previous_candle_triggers_trade)
  {
    ...
    /*if(moves_start_bar_tf==PERIOD_M1) 
...
//|                                                                  |
//+------------------------------------------------------------------+  
ENUM_DIRECTION_BIAS signal_previous_bar_triggered(string instrument,ENUM_TIMEFRAMES bars_timeframe,double min_body_pts,double max_wick_pts)
  {
    if(is_new_M5_bar)
      {
        ENUM_TIMEFRAMES lower_timeframe=-1;
        int start_bar=-1;
        if(bars_timeframe==PERIOD_M5) 
          {
            lower_timeframe=PERIOD_M1;
            start_bar=300; // 300 M1 bars in a M5 bar
          }
        else if(bars_timeframe==PERIOD_H1 && is_new_H1_bar) 
          {
            lower_timeframe=PERIOD_M5;
            start_bar=12; // 12 M5 bars in a H1 bar
          }
        else if(bars_timeframe==PERIOD_D1 && is_new_D1_bar) // note: this will not using the is_new_custom_D1_bar variable
          {
            lower_timeframe=PERIOD_H1;
            start_bar=24; // 24 H1 bars in a D1 bar
          }
        if(lower_timeframe!=-1)
          {
            double high=iHigh(instrument,bars_timeframe,1);
            double low=iLow(instrument,bars_timeframe,1);
            double open=iOpen(instrument,bars_timeframe,1);
            double close=iClose(instrument,bars_timeframe,1);
            bool valid_bull_candle=close-open>=min_body_pts && high-close<=max_wick_pts && open-low<=max_wick_pts;
            bool valid_bear_candle=open-close>=min_body_pts && high-open<=max_wick_pts && close-low<=max_wick_pts;
            int ranges_pivot_bar;
            if(valid_bull_candle) 
              {
                LOP_price=low;
                ranges_pivot_bar=ranges_pivot_bar(BUYING_MODE,instrument,lower_timeframe,start_bar); // get the range's pivot bar since the move's start bar
                LOP_time=iTime(instrument,lower_timeframe,ranges_pivot_bar);
                uptrend=true;
                downtrend=false;
                return DIRECTION_BIAS_BUY;
              }
            else if(valid_bear_candle) 
              {
                HOP_price=high;
                ranges_pivot_bar=ranges_pivot_bar(SELLING_MODE,instrument,lower_timeframe,start_bar); // get the range's pivot bar since the move's start bar
                HOP_time=iTime(instrument,lower_timeframe,ranges_pivot_bar);
                downtrend=true;
                uptrend=false;
                return DIRECTION_BIAS_SELL;
              }
...
double hours_to_roll()
  {
    ...
    if(!hours_to_roll>0)
      {
        print_and_email("Error","H1s_to_roll or minutes_to_roll must be >0 for the algorithm to run.");
        Sleep(1000);
        ExpertRemove();
      }
    return hours_to_roll;
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
ENUM_TIMEFRAMES string_to_timeframe(string timeframe)
  {
    ENUM_TIMEFRAMES timeframe2=-1;
    if(timeframe=="D1" || timeframe=="PERIOD_D1")
      {
      ...
      {
        print_and_email("Error","You cannot type anything other than D1, H1, M5, or M1 into the previous_candle_triggers_trade input box.");
        Sleep(1000);
        ExpertRemove();
      }
    return timeframe2;
...
//|                                                                  |
//+------------------------------------------------------------------+
bool set_moves_start_bar_tf(string timeframe)
  {
    ENUM_TIMEFRAMES timeframe2=string_to_timeframe(timeframe);
    if(timeframe2==PERIOD_M5) moves_start_bar_tf=PERIOD_M1;
    else if(timeframe2==PERIOD_H1) moves_start_bar_tf=PERIOD_M5;
    else
      {
        print_and_email("Error","You cannot type anything other than H1 or M5 into the trade_triggering_timeframe input box.");
        Sleep(1000);
        ExpertRemove();
      }
...
//|                                                                  |
//+------------------------------------------------------------------+
bool set_moves_start_bar(string instrument,ENUM_TIMEFRAMES _moves_start_bar_tf,double _H1s_to_roll,int gmt_offset,double _max_weekend_gap_percent,bool _include_last_week) // TODO: include current_time as a parameter?
  {
    static bool alert_flag=false;
    int _moves_start_bar=-1;
    if(_moves_start_bar_tf==PERIOD_M1) 
        ...
        if(DayOfWeek()==6) // if the broker's server time is Saturday
        ...
        else
          {
            ...
            /*if(uptrend || downtrend)
              ...
              {*/
                int  digits=(int)MarketInfo(instrument,MODE_DIGITS); 
                bool create_object=(display_chart_objects && ObjectFind(move_start_text)<0);
                ...
                if(include_previous_day)
                  {
                    int weeks_start_bar=iBarShift(instrument,_moves_start_bar_tf,weeks_open_time,false);
                    if(create_object) 
                      {
                        double weeks_open_price=iOpen(instrument,_moves_start_bar_tf,weeks_start_bar);
                  ...
                  {
                    int days_start_bar=iBarShift(instrument,_moves_start_bar_tf,days_open_time,false);
                    if(create_object) 
                      {
                        double days_open_price=iOpen(instrument,_moves_start_bar_tf,days_start_bar);
                        ObjectCreate(move_start_text,OBJ_VLINE,0,days_open_time,days_open_price); // it only gets set to these anchors for 1 M5 bar, so it is okay if it is wrong the first bar.
                        ObjectSet(move_start_text,OBJPROP_COLOR,clrWhite);
                      }
              ...
              //}
              ObjectSet(move_start_text,OBJPROP_TIME1,iTime(current_chart,_moves_start_bar_tf,moves_start_bar));
              ObjectSet(move_start_text,OBJPROP_PRICE1,iOpen(current_chart,_moves_start_bar_tf,moves_start_bar));      
          }
      }
    else
      {
...
//|                                                                  |
//+------------------------------------------------------------------+
int ranges_pivot_bar(ENUM_DIRECTIONAL_MODE mode,string instrument,ENUM_TIMEFRAMES timeframe,int start_bar)
  {
    if(mode==BUYING_MODE)
      { 
        int lowest_bar;
    ...
    else if(mode==SELLING_MODE)
      {
        ...
        if(start_bar==0) 
          ...
          highest_bar=iHighest(instrument,timeframe,MODE_HIGH,start_bar,0);
        return highest_bar;
      }
    else 
      return -1;
    ...
    switch(instrument_int)
      {
        ...
        default:
          print_and_email("Error","get_acceptable_spread: "+instrument+"'s maximum spread value is not coded.");
          return false;
      }
  }
//+------------------------------------------------------------------+
...
bool expired_pivot_level(datetime _current_time)
  {
    ...
    int setups_max_time=int((hours_to_roll()*3600)+int(retracement_virtual_expire*3600)+int(trigger_to_peak_max*3600)+361);
...
//|                                                                  |
//+------------------------------------------------------------------+
void ea_retracement_try_to_enter_order(int type,int magic,int max_slippage_pips,string instrument,bool _reduced_risk,int _max_risky_trades,double current_bid)
  {
    color   arrow_color;
    int     digits=(int)MarketInfo(instrument,MODE_DIGITS);
        ...
        if(temp_type==OP_BUY)
          {
            ...
            if(current_chart_matches && display_chart_objects)
              {
                ObjectSet(current_chart+"_HOP",OBJPROP_WIDTH,1);
                Comment("uptrend SIGNAL(not order) was last");
        ...
        else if(temp_type==OP_SELL)
          {
            ...
            if(current_chart_matches && display_chart_objects)
              {
                ObjectSet(current_chart+"_LOP",OBJPROP_WIDTH,1);
                Comment("downtrend SIGNAL(not order) was last");
...
//|                                                                  |
//+------------------------------------------------------------------+
void ea_reversal_try_to_enter_order(int type,int magic,int max_slippage_pips,string instrument,bool _reduced_risk,int _max_risky_trades,double current_bid)
  {
    color   arrow_color;
    int     digits=(int)MarketInfo(instrument,MODE_DIGITS);
    double  pending_order_distance_pts=0, // keep at 0
            periods_pivot_price=0,
            point=MarketInfo(instrument,MODE_POINT),
            takeprofit_pts=0,
            stoploss_pts=0,
            _pullback_percent=pullback_percent,
    ...
    if(type==OP_BUY)
    ...
    else if(type==OP_SELL)
      {
        ...
        if(HOP_price>0) periods_pivot_price=HOP_price;
      }
    else 
      {
        print_and_email("Error","The algorithm programmer can only use OP_BUY and OP_SELL for signals.");
    ...
    stoploss_pts=NormalizeDouble(periods_range_pts*stoploss_percent,digits);
    print_and_email_margin_info(instrument,"Margin Info Before Trying To Enter Trade");
    int ticket=check_for_entry_errors (instrument,
                                       type,
                                       lots,
    ...
    if(takeprofit_pts_is_ma_distance_based && moving_avg_period>0 && takeprofit_percent>0)
      {
        ...
        _takeprofit_percent=MathMin(NormalizeDouble(takeprofit_percent*distance_percent,2),takeprofit_percent)+takeprofit_percent;
        Print("new_takeprofit_percent: ",DoubleToString(_takeprofit_percent));
...
//|                                                                  |
//+------------------------------------------------------------------+
void set_new_active_trade_expire(double new_takeprofit_percent)
  {
    if(active_trade_expire_is_tp_based)
      {
        if(takeprofit_percent>0)
          {
            double change_percent=MathMin(NormalizeDouble(new_takeprofit_percent/takeprofit_percent,2),2.00); // do not let the value be change to more than double (2.00 aka 200%)
            if(change_percent<=1) return; // do not let the value be change to less than what it was originally
...
//|                                                                  |
//+------------------------------------------------------------------+
void send_and_get_order_ticket_test()
  {
...
int send_and_get_order_ticket(string instrument,int cmd,double lots,double _distance_pts,double periods_pivot_price,double sl_pts,double tp_pts,int max_slippage,double spread_pts,double range_pts,string _EA_name=NULL,int magic=0,int expire=0,color a_clr=clrNONE,bool _market_exec=false) // the "market" argument is to make this function compatible with brokers offering market execution. By default, it uses instant execution.
  {
    ...
    //send_and_get_order_ticket_test();
                ...
                if(order_type==OP_BUY || order_type==OP_BUYLIMIT)
                  {
                    ...
                    uptrend_order_was_last=true;
                    ObjectSet(current_chart+"_HOP",OBJPROP_WIDTH,1);
                    Comment("uptrend order was last");
                ...
                else if(order_type==OP_SELL || order_type==OP_SELLLIMIT)
                  {
                    ...
                    downtrend_order_was_last=true;
                    ObjectSet(current_chart+"_LOP",OBJPROP_WIDTH,1);
                    Comment("downtrend order was last");
    ...
    if(ticket>0) 
      {
        ...
        string email_body=StringConcatenate(subject,
                                            ...
                                            ", spread_points: ",DoubleToString(spread_pts,digits),
